...
$Ash = 15;//date('j', strtotime('First Wednesday of March'. $fYear));
$GoodFriday = 30;
$Easter = 1;//date('j', strtotime('Last Sunday of March '.$fYear));
$Palm = 25; //date('j', strtotime('-7 day Last Sunday of March'. $fYear));
...
var $NewYears = "'.$NewYearDay.'";					
		if ($NewYears === "Sat"){
				$("[id^=Jan_'.$NewYear.'_]").html(" '.$NewYear.') <p> Happy New Year <br/><font size=\"1\"></p>");
				...
				$("[id^=Jan_'.$NewYear.'_]").html(" '.$NewYear.') <p> Happy New Year <br/><font size=\"1\"> Official Holiday </p>");
		}										
  ...
  $("[id^=Feb_'.$Ash.'_]").html(" '.$Ash.') <p> Ash Wednesday <br/><font size=\"1\"> (First day of Lent)</font> </p>");
  $("[id^=Mar_'.$Palm.'_]").html(" '.$Palm.') <p> Palm Sunday <br/><font size=\"1\"> </p>");
  $("[id^=Mar_'.$GoodFriday.'_]").html(" '.$GoodFriday.') <p> Good Friday <br/><font size=\"1\"> </p>");
...
function populateEvents($event, $type, $attach, $day){   
	 ...
	 if (strlen(strstr($agent,"Firefox")) > 0 ){      
         ...
         echo '<script> 
	 $(document).ready(function(){
	 $("#Calendar").ready(function(){ '. ((!empty($attach)) ? '$("#'.$date.'").append(" <div style=\"font-weight:normal\">'.$event.' <br/> <img src=\"./images/'.$type.'.png\" width=\"30pixels\" onclick=\"window.location.href= \'./images/'.$attach.'\'; \" \"><br/><small style=\"font-size:8px;\">'.((strlen($attach) > 35) ? substr($attach, 0, 30)."...$type": $attach) .'</small> </div>");});}); </script>' : '$("#'.$date.'").append(" <div style=\"font-weight:normal\">'.$event.'<br/><small style=\"font-size:8px\">'.$type.'</small></div>");});}); </script>')  ;	    	
}

$std = $db->prepare("SELECT event, type, attachment, date from nbdl_eventCalendar where date is not null or '0000-00-00' ");
$std->execute();
